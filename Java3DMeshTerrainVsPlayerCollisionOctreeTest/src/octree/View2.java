...
public class View2 extends JPanel {
    ...
    public void start() {
        ...
        try {
            ...
            terrain.load("/res/princess_peaches_castle_(outside).obj", 20, 0, -5, 0);
        } catch (Exception ex) {
    ...
    private void draw(Graphics2D g) {
        ...
        // front
        g.setTransform(at);
        g.translate(getWidth() / 4, getHeight() / 1.25);
        g.setColor(Color.BLACK);
        g.drawString("Front", 0, -280);
        g.scale(1, -1);
        drawFront(g);

        // top
        g.setTransform(at);
        g.translate(2.75 * getWidth() / 4, getHeight() / 1.25);
        g.setColor(Color.BLACK);
        g.drawString("Top", 0, -280);
        g.scale(1, 1);
        drawTop(g);
        ...
        // retrieved faces
        g.drawString("retrieved faces = " + retrievedFaces.size()  + " / " + terrain.getFaces().size(), 50, 90);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            frame.setTitle("Java Octree Spatial Partition Test");
